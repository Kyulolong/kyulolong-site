 ...
 *
 * 라이트 테마에서는 이 축이 약해진다 — plate(#ebe7f5)와 surface(#faf9fd)가 1.14:1 이라
 * "불 켜진 도판"이 아니고, iris-wash 도 옅은 보라 면이 된다. 의도된 것이다: 서비스와
 * 영상은 같은 목록에 섞이지 않고(대문엔 영상이 없다), 여백 액자(영상)와 꽉 찬
 * 도판(일러스트)이라는 **구조** 차이가 남는다. plate 는 라이트에서도 그대로다 —
 * multiply 는 밝은 판에서만 살고, 라이트 시절에도 이 판이었다.
 */
const TONE = {
