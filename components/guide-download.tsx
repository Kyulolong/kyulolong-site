...
export function GuideDownload() {
  ...
  return (
    <section id="goal-guide" className="mx-auto w-full max-w-[1120px] px-6 py-8 sm:px-8 sm:py-12">
      <div className="bg-iris-wash border-line grid overflow-hidden rounded-[2rem] border lg:grid-cols-[1.1fr_0.9fr]">
        <div className="px-7 py-10 sm:px-10 sm:py-14 lg:px-14 lg:py-16">
          <p className="text-iris-soft text-sm font-bold tracking-[0.08em]">무료 가이드 · 01</p>
          ...
          <p className="text-ink-soft mt-6 max-w-[34rem] text-lg leading-relaxed text-pretty">
            좋은 목표의 조건부터 KPI·OKR·MBO의 차이, 수립 이후의 운영까지 한 권에
            정리했습니다. 팀의 목표를 세우거나 다시 점검할 때 바로 꺼내 쓰세요.
          </p>
          <ul className="text-ink-soft mt-8 grid gap-3 text-sm sm:grid-cols-2">
            <li>✓ 목표 설계의 기준과 점검 질문</li>
            <li>✓ 제도별 차이와 실제 예시</li>
            <li>✓ 수립 이후 운영 원칙</li>
            <li>✓ 실무에 바로 쓰는 체크리스트</li>
          </ul>
        </div>
        ...
        <div className="border-line bg-canvas border-t px-7 py-10 sm:px-10 sm:py-14 lg:border-t-0 lg:border-l lg:px-12 lg:py-16">
          ...
          <p className="text-ink-faint mt-2 text-sm">성함과 이메일만 남기면 바로 보내드립니다.</p>
            ...
            <form onSubmit={submit} className="mt-8" noValidate>
              <div className="grid gap-5">
                <label className="grid gap-2 text-sm font-bold">
                  ...
                  <input
                    ...
                    autoComplete="name"
                    placeholder="홍길동"
                    className="border-line-strong bg-canvas text-ink placeholder:text-ink-faint min-h-13 rounded-xl border px-4 py-3 font-normal outline-none transition-shadow focus:ring-2 focus:ring-[var(--color-iris)]"
                  />
                </label>
                <label className="grid gap-2 text-sm font-bold">
                  이메일
              ...
              <div className="text-ink-soft mt-6 grid gap-4 text-sm leading-relaxed">
                ...
                <label className="flex items-start gap-3">
                  ...
                  <span>
                    <strong className="text-ink">[선택]</strong> Proof 컨설팅과 다음 가이드 소식을
                    이메일로 받겠습니다.
                  </span>
                </label>
              </div>
              ...
              {status === "error" ? (
                <p className="mt-5 text-sm text-red-600" role="alert">{message}</p>
              ) : null}

              <button
                type="submit"
                disabled={status === "submitting"}
                className="bg-ink text-canvas mt-7 inline-flex min-h-13 w-full items-center justify-center rounded-full px-6 py-3 font-bold transition-opacity hover:opacity-85 disabled:cursor-wait disabled:opacity-60"
              >
                {status === "submitting" ? "보내는 중…" : "무료 가이드 받기"}
              </button>
