...
| 자리 | 무엇을 쓰나 |
| --- | --- |
| 앱 안 헤더 마크 | **하우스 마크 — 말풍선과 스파크** (`public/brand/mark-on-dark.svg`). `docs/DESIGN.md` §7 — 서비스마다 로고를 새로 그리지 않는다 |
| 탭 · 홈 화면 아이콘 | **서비스 고유 그림.** `wave-sound` 의 달+파도, `prompt` 의 대본 줄 |

§7 이 금지하는 건 **워드마크 앞에 붙는 마크**다. 파비콘은 별개 슬롯이라, 앱은 헤더에
하우스 마크를 쓰면서 탭에는 자기 그림을 단다.

> ### ⚠️ 새 서비스는 반드시 **새 마크**(말풍선+스파크)를 쓴다
>
> 마크는 2026-09 에 **ㄱ+점 → 말풍선과 스파크**로 바뀌었다. `wave-sound` 처럼 그 전에
> 나간 서비스에는 아직 옛 ㄱ+점이 박혀 있고, **소급해서 고치지 않는다.**
>
> 그래서 **먼저 만든 서비스를 베끼면 옛 마크를 그대로 가져오게 된다.** 실제로
> `trick` 을 만들 때 `wave-sound` 의 data URI 를 복사해 와서 한 번 틀렸다.
> 참고할 곳은 먼저 나간 서비스가 아니라 **`public/brand/` 의 파일**이다.
>
> | 바탕 | 파일 | 말풍선 색 |
> | --- | --- | --- |
> | 어두운 앱, 마크가 **글자 옆**에 설 때 | `public/brand/mark-on-dark.svg` | `#edebf5` |
> | 밝은 앱, 마크가 **글자 옆**에 설 때 | `public/brand/mark-on-light.svg` | `#121019` |
> | 마크가 **혼자** 설 때 (바탕을 모를 때) | `public/brand/mark.svg` | `#6332EB` |
>
> 스파크 세 획은 어느 변형에서나 `#8FFF00` 그대로다. 고르는 기준은 바탕이 아니라
> **옆에 글자가 있느냐**이고, 글자 옆에 설 때는 그 글자와 같은 색이어야 한 덩어리로
> 읽힌다 (§7).
>
> 빌드가 없는 단일 HTML 앱은 data URI 로 인라인한다. 그럴 때는 `#` 를 반드시 `%23` 로
> 인코딩하고 (CSS 에서 `#` 는 프래그먼트로 잘린다), 원본 SVG 로 되돌려 대조해서
> 넣는다. 그리고 **출처를 주석으로 남긴다** — 마크가 또 바뀔 때 찾을 수 있어야 한다.
>
> - [ ] 마크를 `public/brand/` 에서 가져왔다 (다른 서비스에서 복사하지 않았다)
> - [ ] 글자 옆이면 글자와 같은 색 변형을 골랐다
> - [ ] data URI 라면 원본과 대조했고 출처 주석을 남겼다
