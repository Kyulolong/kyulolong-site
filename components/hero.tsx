  ...
  return (
    ...
    <section className="mx-auto w-full max-w-[1120px] px-6 pt-20 pb-4 sm:px-8 sm:pt-28 sm:pb-6">
      <div className="max-w-[46rem]">
        {/* 문구는 인스타 썸네일·소개글에 이미 박혀 있는 그 줄이다. 릴스를 보고 넘어온
            사람이 같은 얼굴을 만나야 한다. 여기만 다른 말을 쓰면 채널이 둘로 갈린다.

            세 칸이 각각 다른 일을 한다 — 정체성 · 시작점 · 도달점. 그리고 어미가
            `가 / 에 / 지` 로 전부 갈린다. 같은 어미로 끝나는 칸이 둘만 생겨도
            문장이 아니라 나열로 읽히므로, 손볼 때 어미부터 볼 것.

            ⚠️ 예전엔 마지막 칸이 `1시간` 이었다. 여기서 뺀 이유는 이 줄에 **잴 대상이
            없어서**다. 카드의 buildTime 은 제목·태그라인 바로 아래 붙어 "그 서비스가
            2시간"이지만, 히어로의 맨 숫자는 걸릴 데가 없어서 빠름이 아니라 가벼움으로
            읽힌다. 그래서 "얼마나 빨리"를 "얼마나 멀리"로 바꿨다 — 아래 서브카피의
            "앱스토어에 올라간 것도 있습니다"가 이 줄을 받는다. 그 문장을 지우면
            이 줄이 근거를 잃는다.
