 ...
 */
import { ContentError, getServices, getThoughts, getVideos, validateContent } from "../lib/content";
...
try {
  ...
  console.log(
    `✓ 콘텐츠 검증 통과 — 서비스 ${services.length}개, 영상 ${videos.length}개, ` +
      `글 ${thoughts.length}개, 양방향 연결 ${links}쌍`,
  );
} catch (error) {
